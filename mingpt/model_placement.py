...
class GPT(nn.Module):
    ...
    def __init__(self, config, exp_config):
        ...
        # exp related
        self.num_features = exp_config.num_features
        self.num_mfeatures = exp_config.num_meta_features
        self.chassis_dimx = exp_config.chassis_dim[0]
    ...
    def forward(self, states, actions, targets=None, rtgs=None, 
        ...
        x = self.drop(token_embeddings) # (batch, 1 + 3*context, n_Embedding)
        x = self.blocks(x)
        x = self.ln_f(x)
